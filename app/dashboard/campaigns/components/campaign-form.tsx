...
import { ChevronLeft, ChevronRight, Check, Users, Hash, Clock, Filter, CalendarIcon, Plus, Send, Loader2, MessageSquare } from "lucide-react";
...
export function CampaignForm({ initialData, tags, templates, initialContactCount }: CampaignFormProps) {
    const router = useRouter();
    const toast = useToast();
    const [currentStep, setCurrentStep] = useState(1);
    const [loading, setLoading] = useState(false);
    const [estimatedCount, setEstimatedCount] = useState<number>(initialContactCount);

    // Form State
    const [formData, setFormData] = useState({
        name: initialData?.name || "",
        audienceType: (initialData?.audienceType || "all") as "all" | "tags" | "count" | "recent" | "active",
        includedTags: initialData?.includedTags || [] as string[],
        contactLimit: initialData?.contactLimit || 100,
        recentDays: initialData?.recentDays || 7,
        messageType: initialData?.messageType || "text",
        messageContent: initialData?.messageContent || "",
        templateId: initialData?.templateId || "",
        scheduleType: initialData?.status === "scheduled" ? "later" : (initialData?.status === "sending" ? "send" : "now") as "now" | "later" | "send",
        scheduledAt: initialData?.scheduledAt ? initialData.scheduledAt.toISOString() : "",
    });
    ...
    useEffect(() => {
        ...
        async function updateCount() {
            ...
            const result = await getContactCount({
                ...
                activeOnly: formData.audienceType === "active" ? true : undefined
            });
        ...
        updateCount();
    }, [formData.audienceType, formData.includedTags, formData.contactLimit, currentStep]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (result.success) {
                toast.success(result.message || "تم حفظ الحملة بنجاح");
    ...
    return (
        <div className="max-w-4xl mx-auto pb-20">
            <h1 className="text-2xl font-bold mb-6">{initialData ? 'تعديل الحملة' : 'إنشاء حملة جديدة'}</h1>
                ...
                {currentStep === 2 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <button
                                onClick={() => setFormData({ ...formData, audienceType: "active" })}
                                className={`p-5 rounded-xl border-2 text-right transition-all ${formData.audienceType === "active" ? "border-primary bg-primary/5" : "border-gray-100 hover:border-gray-200"}`}
                            >
                                <MessageSquare className={`w-6 h-6 mb-2 ${formData.audienceType === "active" ? "text-primary" : "text-gray-400"}`} />
                                <div className="font-bold mb-1">متفاعلون (٢٤ ساعة)</div>
                                <p className="text-sm text-gray-500">من لديهم نشاط آخر ٢٤ ساعة</p>
                            </button>
                        </div>
                        ...
                        {formData.messageType === "text" ? (
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    ...
                                    {templates.map(t => (
                                        <option key={t.id} value={t.id}>{t.name} ({t.language || 'ar'})</option>
                                    ))}
                                </select>
                            ...
                            {formData.scheduleType === "later" && (
                                <div className="mr-8 animate-in slide-in-from-top-2 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="time"
                                            value={formData.scheduledAt && !isNaN(Date.parse(formData.scheduledAt)) ? format(new Date(formData.scheduledAt), "HH:mm") : ""}
                                            onChange={(e) => {
                                                const [hours, minutes] = e.target.value.split(":").map(Number);
                                                const date = formData.scheduledAt && !isNaN(Date.parse(formData.scheduledAt)) ? new Date(formData.scheduledAt) : new Date();
                                                date.setHours(hours);
                                        ...
                                        {formData.audienceType === "all" ? "الجميع" :
                                            formData.audienceType === "tags" ? `${formData.includedTags.length} وسوم` :
                                                formData.audienceType === "count" ? `أول ${formData.contactLimit} جهة` :
                                                    formData.audienceType === "recent" ? `آخر ${formData.recentDays} يوم` :
                                                        "متفاعلون (٢٤ ساعة)"}
                                    </span>
                                </div>
                                <div className="flex justify-between">
